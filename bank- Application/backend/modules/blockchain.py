...
class Blockchain:
    ...
    def get_latest_block(self) -> Dict:
        ...
        try:
            result = self.supabase.table("blocks").select("*").order("index", desc=True).limit(1).execute()
            if result.data:
    ...
    def save_block(self, block: Block) -> bool:
        ...
        try:
            ...
            result = self.supabase.table("blocks").insert(block_data).execute()
    ...
    def validate_chain(self) -> Dict:
        ...
        try:
            ...
            result = self.supabase.table("blocks").select("*").order("index").execute()
            blocks = result.data
